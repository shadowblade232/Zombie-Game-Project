...
namespace EasyUIStyle.UI
{
    ...
    [CustomEditor(typeof(EasyUIToggle))]
    public class EasyUIToggleInspector : UnityEditor.UI.ToggleEditor
    {
        private EasyUIToggle _target;
        ...
        {
            _target = (EasyUIToggle)target;

            if (EasyUIComponentCore<EasyUI_ToggleStyle>.easyUI_Data == null)
                    _target.easyUICore.Initialize();

            styleList = EasyUI_HelperFunctions.GetStyles<EasyUI_ToggleStyle>();
            ...
            if (GUI.changed)
                ...
                {
                    ((EasyUIToggle)obj).easyUICore.styleIndex = serializedObject.FindProperty("styleIndex").intValue;
                }
