...
namespace EasyUIStyle.UI
{
    ...
    [CustomEditor(typeof(EasyUIDropdown))]
    public class EasyUIDropdownInspector : UnityEditor.UI.DropdownEditor
    {
        private EasyUIDropdown _target;
        ...
        {
            _target = (EasyUIDropdown)target;

            if (EasyUIComponentCore<EasyUI_DropdownStyle>.easyUI_Data == null)
                    _target.easyUICore.Initialize();

            styleList = EasyUI_HelperFunctions.GetStyles<EasyUI_DropdownStyle>();
            ...
            if (GUI.changed)
                ...
                {
                    ((EasyUIDropdown)obj).easyUICore.styleIndex = serializedObject.FindProperty("styleIndex").intValue;
                }
