...
namespace EasyUIStyle.UI
{
    ...
    [CustomEditor(typeof(EasyUITMProInput))]
    public class EasyUITMProInputInspector : TMPro.EditorUtilities.TMP_InputFieldEditor
    {
        private EasyUITMProInput _target;
        ...
        {
            _target = (EasyUITMProInput)target;

            if (EasyUIComponentCore<EasyUI_TMPInputStyle>.easyUI_Data == null)
                    _target.easyUICore.Initialize();

            styleList = EasyUI_HelperFunctions.GetStylesTMP<EasyUI_TMPInputStyle>();
            ...
            if (GUI.changed)
                ...
                {
                    ((EasyUITMProInput)obj).easyUICore.styleIndex = serializedObject.FindProperty("styleIndex").intValue;
                }
