...
namespace EasyUIStyle.UI
{
    ...
    [CustomEditor(typeof(EasyUIInputField))]
    public class EasyUIInputInspector : UnityEditor.UI.InputFieldEditor
    {
        private EasyUIInputField _target;
        ...
        {
            _target = (EasyUIInputField)target;

            if (EasyUIComponentCore<EasyUI_InputStyle>.easyUI_Data == null)
                    _target.easyUICore.Initialize();

            styleList = EasyUI_HelperFunctions.GetStyles<EasyUI_InputStyle>();
            ...
            if (GUI.changed)
                ...
                {
                    ((EasyUIInputField)obj).easyUICore.styleIndex = serializedObject.FindProperty("styleIndex").intValue;
                }
