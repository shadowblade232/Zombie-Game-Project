...
namespace EasyUIStyle.UI
{
    ...
    [CustomEditor(typeof(EasyUIText))]
    public class EasyUITextInspector : UnityEditor.UI.TextEditor
    {
        private EasyUIText _target;
        ...
        {
            _target = (EasyUIText)target;

            if (EasyUIComponentCore<EasyUI_TextStyle>.easyUI_Data == null)
                    _target.easyUICore.Initialize();

            styleList = EasyUI_HelperFunctions.GetStyles<EasyUI_TextStyle>();
            ...
            if (GUI.changed)
                ...
                {
                    ((EasyUIText)obj).easyUICore.styleIndex = serializedObject.FindProperty("styleIndex").intValue;
                }
